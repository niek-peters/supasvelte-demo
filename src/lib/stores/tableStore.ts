...
> extends Readable<Entries> {
	...
	mutate(this: void, id: string | number, value: MutateEntry): Promise<PostgrestError | null>;
	tableName: string;
	indexName: string;
...
>(
	...
	const store: Writable<Entry[]> = writable([], () => {
		const tableStore = store as unknown as TableStore<Entry[], NewEntry, MutateEntry>;
	...
	const mutate = async (id: string | number, value: MutateEntry) => {
		...
		if (
			...
			if (
				...
				now.getTime() - tableStore.lastMutate.getTime() < tableStore.mutateInterval
			) {
		...
		const savedIds = await saveUnsavedEntries(supabase, tableStore, tableName, indexName);
...
async function saveUnsavedEntries<Entry extends TableRow>(
	...
	if (tableStore.unsavedIds && tableStore.unsavedIds.length) {
		...
		tableStore.unsavedIds = tableStore.unsavedIds.filter((id) => !savedIds.includes(id));
	}
...
function closeConnections<Entry extends TableRow>(
	...
	if (tableStore.broadcastChannel && tableStore.broadcastChannel.state === 'joined') {
